...
|*referenceBases**|Reference bases for this variant (starting from `start`). Accepted values: [ACGTN]* N is a wildcard, that denotes the position of any base.|string|`'G'`|
|*assemblyId* *|Assembly identifier|string|`'GRCh38'`|
...
|alternateBases~|The bases that appear instead of the reference bases. Accepted values: [ACGTN]* N is a wildcard, that denotes the position of any base.<br /> <br/>Symbolic ALT alleles (DEL, INS, DUP, INV, CNV, DUP:TANDEM, DEL:ME, INS:ME) will be represented in `variantType`.<br/> <br/> See the ALT field in [VCF 4.2 specification](https://samtools.github.io/hts-specs/VCFv4.2.pdf)<br/> <br/>*Either `alternateBases` OR `variantType` is REQUIRED*|string|`'A'`, `'AGATAC'`|
